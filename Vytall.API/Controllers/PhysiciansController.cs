...
namespace Vytall.API.Controllers
{
    ...
    public class PhysiciansController : ControllerBase
    {
        ...
        public PhysiciansController(VytallContext context)
        {
            _context = context;
        }
        ...
        // ✅ GET SINGLE PHYSICIAN (Admins and Physician can fetch their own profile)
        [HttpGet("{id}")]
        [Authorize(Roles = "Admin,Physician")]
        public async Task<ActionResult<PhysicianDTO>> GetPhysicianById(int id)
        {
            var userRole = User.FindFirst(ClaimTypes.Role)?.Value;
            var userPhysicianId = User.FindFirst("physicianId")?.Value;

            if (userRole == "Physician")
            {
                if (string.IsNullOrEmpty(userPhysicianId) || userPhysicianId != id.ToString())
                    return Forbid();
            }
        ...
        public async Task<ActionResult<IEnumerable<PrescriptionDTO>>> GetPrescriptionsByPhysician(int id)
        {
            var userRole = User.FindFirst(ClaimTypes.Role)?.Value;
            var userPhysicianId = User.FindFirst("physicianId")?.Value;
            ...
            var prescriptions = await _context.Prescriptions
                ...
                .Select(p => new PrescriptionDTO
                {
                    ...
                    PhysicianName = p.Physician != null ? p.Physician.FirstName + " " + p.Physician.LastName : "Unknown",
                    ...
                    Status = p.ExpirationDate >= now ? "Active" : "Expired"
                })
                .ToListAsync();
