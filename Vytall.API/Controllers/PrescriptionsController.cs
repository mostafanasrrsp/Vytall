...
namespace Vytall.API.Controllers
{
    ...
    [Authorize(Roles = "Patient, Admin, Facility, Physician, Pharmacist")]
    public class PrescriptionsController : ControllerBase
    {
        private readonly VytallContext _context;

        public PrescriptionsController(VytallContext context)
        {
            _context = context;
        }
        ...
        [Authorize(Roles = "Admin, Facility, Physician, Pharmacist")]
        ...
        public async Task<IActionResult> GetPrescriptionReminders(int patientId)
        {
            ...
            // Only allow patients to access their own reminders
        ...
        // Automated Refill Request
        [HttpPost("refill-request")]
        [Authorize(Roles = "Patient")]
        public async Task<ActionResult<RefillRequestResponseDTO>> RequestRefill([FromBody] RefillRequestDTO request)
        {
            // Check if prescription exists and belongs to the patient
            var prescription = await _context.Prescriptions
                .FirstOrDefaultAsync(p => p.PrescriptionId == request.PrescriptionId && p.PatientId == request.PatientId);
            if (prescription == null)
            {
                return NotFound(new RefillRequestResponseDTO
                {
                    Status = "error",
            ...
            // Here you would integrate with pharmacy or refill logic
            // For now, just mock a successful request
            return Ok(new RefillRequestResponseDTO
            {
                Status = "success",
                Message = $"Refill request for prescription {request.PrescriptionId} has been received."
            });
        }
    }
}