...
export default function MedicalRecordsManager({ patientId }) {
  const { user } = useAuth();
  ...
  const [formData, setFormData] = useState({
    patientId: patientId || (isPatient ? String(user.patientId) : ""),
  ...
  useEffect(() => {
    // If patientId is provided, always load that patient's records
    if (patientId) {
      loadRecordsForPatient(patientId);
    } else if (isPatient) {
      loadRecordsForPatient(user.patientId);
    } else {
      loadRecords();
    }
    // For non-patient users and when no specific patientId is provided, load the full patient map
    if (!isPatient && !patientId) loadPatients();
    // For Admins only, load the physician options
    if (isAdmin) loadPhysicians();
  }, [patientId, isPatient, isAdmin]);
  ...
  async function loadRecordsForPatient(id) {
    try {
      const data = await fetchMedicalRecordsByPatient(id);
      setRecords(data);
    } catch (error) {
  ...
  async function handleSubmit(e) {
    ...
    const dto = {
      ...
      // Only include physician data for non-patient users
      physicianId: isPhysician ? Number(user.physicianId) : null,
      physicianName: null,
    ...
    try {
      ...
      isPatient ? loadRecordsForPatient(dto.patientId) : loadRecords();
    } catch (error) {
  ...
  async function handleDelete(id) {
    try {
      ...
      isPatient ? loadRecordsForPatient(user.patientId) : loadRecords();
    } catch (error) {
  ...
  return (
    <div className="flex flex-col w-full">
      <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-4xl">
        <h2 className="text-2xl font-bold mb-4">{editingId ? "Edit Medical Record" : "Add Medical Record"}</h2>
        <form onSubmit={handleSubmit} className="space-y-4 mb-8">
          {/* Show PatientSelect only when no patientId is provided and user is not a patient */}
          {!patientId && !isPatient && (
            <PatientSelect
          ...
          {/* Physician selection is available only for Admins */}
          {isAdmin && (
            <>
              <Select
